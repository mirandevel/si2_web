<div>
    <div class="container antialiased font-sans mx-auto px-4 sm:px-8">
        <div class="py-8">
            <div>
                <h2 class="text-2xl font-semibold leading-tight">Promociones</h2>
            </div>
                ...
                @foreach($categorias as $categoria)
                    <div class="border-2 border-paleta-4  p-5 rounded bg-white">
                        <div class="relative flex">
                            <p class="text-paleta-4 text-2xl">{{ucfirst($categoria->nombre)}}</p>
                            <p class="text-paleta-7 text-3xl absolute right-0">-{{$categoria->descuento}}%</p>
                        </div>

                        <p class="text-black text-2xl">{{ucfirst($categoria->descripcion)}}</p>
                        <div class="flex justify-between pt-5  ">
                            ...
                            <button type="button" wire:click="cargarDatosDelFormularioEdit({{ $categoria->id }}, '{{ $categoria->nombre }}','{{ $categoria->descripcion }}','{{ $categoria->descuento}}')" onclick="mostrarModalEdit()">
...
{{--                            </button>--}}
    ...
    <div wire:ignore.self class="modal-create h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        ...
        <div class="bg-white rounded shadow-lg w-1/3 border border-gray-200 rounded-lg overflow-hidden">
            ...
            <div class="border-b px-4 py-2 flex justify-between items-center">
                <h3 class="font-semibold text-lg">Crear Promoción</h3>
                    ...
                    <div class="p-5 pb-6">
                        <div class="flex flex-wrap">
                            <div class="w-full px-3">
                                ...
                                <input type="text" wire:model="descripcion" class="block w-full px-4 py-3 mb-3 leading-tight text-gray-700 bg-gray-200 border border-gray-200 rounded appearance-none focus:outline-none focus:bg-white focus:border-gray-500">

                                <label class="block mb-2 text-xs font-bold tracking-wide text-gray-700 uppercase"
                                       for="grid-email">
                                    Descuento (%)
                                </label>
                                <input type="text" wire:model="descuento" class="block w-full px-4 py-3 mb-3 leading-tight text-gray-700 bg-gray-200 border border-gray-200 rounded appearance-none focus:outline-none focus:bg-white focus:border-gray-500">
                            </div>
                        </div>
                        @error('nombre')<span class="error">{{ $message }}</span><p>error</p>@enderror
                        @error('descripcion')<span class="error">{{ $message }}</span><p>error</p>@enderror
                        @error('descuento')<span class="error">{{ $message }}</span><p>error</p>@enderror
                    </div>
    ...
    <div wire:ignore.self class="modal-edit h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        ...
        <div class="bg-white rounded shadow-lg w-1/3 border border-gray-200 rounded-lg overflow-hidden">
            <!-- modal header -->
            <div class="border-b px-4 py-2 flex justify-between items-center">
                <h3 class="font-semibold text-lg">Editar Promoción</h3>
    ...
    <div wire:ignore.self class="modal-delete h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        ...
        <div class="bg-white rounded shadow-lg w-1/3 border border-gray-200 rounded-lg overflow-hidden">
            <!-- modal header -->
            <div class="border-b px-4 py-2 flex justify-between items-center">
                <h3 class="font-semibold text-lg">Eliminar Promoción</h3>
